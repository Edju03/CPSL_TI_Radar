...
def read_adc_data(adc_data_bin_file, mmwave_device):
    ...
    expected_size = num_samples * num_chirps * num_frames * num_rx
...
def plot_adc_data(ax, line_real, line_imag, adc_data, num_samples, num_chirps, desired_frame, desired_chirp, rx_channel, frame_periodicity, sample_rate, idle_time, adc_start_time, ramp_time, freq):
    global timestamps
    ...
    time_indices = desired_frame * frame_periodicity + desired_chirp * (idle_time + ramp_time) + (idle_time + adc_start_time) + t * time_per_sample

    complex_magnitude = np.abs(data)
    
    thresholds = (np.mean(complex_magnitude) + 5 * np.std(complex_magnitude)) * 1.5

    for magnitude, time_index in zip(complex_magnitude, time_indices):
        if magnitude > thresholds:
            if time_index not in timestamps:
    ...
    ax.set_title(f'ADC data for Frame: {desired_frame}, Chirp: {desired_chirp}')  
    ax.relim()
    ax.autoscale_view()
...
def predict_next_interference(timestamps, unique_periods, counts, frame_periodicity, idle_time, adc_start_time):
    if len(unique_periods) > 0 and counts.max() >= 3:  
        ...
        if start_time <= next_interference_time <= end_time:
            return next_interference_time, lower_bound, upper_bound
...
def plot_prediction(ax, next_interference_time, lower_bound, upper_bound, timestamps, frame_periodicity, idle_time, adc_start_time, unique_periods, counts):
    global prev_interference_times, prev_bounds
    ax.clear()
    current_time = timestamps[-1] if timestamps else 0
    ax.plot(timestamps, np.ones_like(timestamps), 'o', label='Interference Times')

    for interference_time, bounds in zip(prev_interference_times, prev_bounds):
        if interference_time is not None:
            ax.axvline(interference_time, color='g', linestyle='--')
            if bounds:
                ...
                if good_range_start < good_range_end:
                    ax.fill_betweenx([0.8, 1.2], good_range_start, good_range_end, color='g', alpha=0.3)

    if next_interference_time is not None:
        ax.axvline(next_interference_time, color='r', linestyle='--', label='Predicted Next Interference')
        ...
        if good_range_start < good_range_end:
            ax.fill_betweenx([0.8, 1.2], good_range_start, good_range_end, color='r', alpha=0.3, label='Prediction Range')
        prev_interference_times.append(next_interference_time)
        prev_bounds.append((lower_bound, upper_bound))
    ...
    ax.set_xlim([max(0, current_time - window_size), current_time + window_size/10])
...
def main():
    adc_data_bin_file = '/Users/edwardju/Downloads/adc_data_FreqTest2.bin'
    mmwave_setup_json_file = '/Users/edwardju/Downloads/FreqTest2.mmwave.json'
    ...
    num_frames = mmwave_device.num_frame
    num_rx = mmwave_device.num_rx_chnl
    win_hann = mmwave_device.win_hann
    dbfs_coeff = mmwave_device.dbfs_coeff
    range_res = mmwave_device.range_res
    frame_periodicity = mmwave_device.frame_periodicity 
    idle_time = mmwave_device.chirp_idle_time / 1000
    adc_start_time = mmwave_device.chirp_adc_start_time / 1000
    sample_rate = mmwave_device.adc_samp_rate * 1000
    ramp_time = mmwave_device.chirp_ramp_time / 1000
    freq = mmwave_device.freq / 1e9

    range_data = np.zeros((num_samples, num_chirps, num_rx), dtype=np.float32)
    ...
    next_interference_time = None

    plt.ion()
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 12))
    
    line1, = ax1.plot(np.zeros(num_samples))
    ax1.set_title('Range FFT')
    ax1.set_xlabel('Range (m)')
    ax1.set_ylabel('Magnitude (dB)')

    line_real, = ax2.plot(np.zeros(num_samples), label='real')
    line_imag, = ax2.plot(np.zeros(num_samples), label='imaginary')
    ...
    line_real_chirp, = ax_chirp.plot(np.zeros(num_samples), label='real')
    line_imag_chirp, = ax_chirp.plot(np.zeros(num_samples), label='imaginary')
    ...
    try:
        ...
        while plt.fignum_exists(fig.number) and plt.fignum_exists(fig_chirp.number):
            if running:
                range_fft(adc_data[:, :, frame_index, :], range_data, num_samples, num_chirps, num_rx, win_hann, dbfs_coeff)

                range_magnitude = range_data[:, :, rx_channel]
                line1.set_ydata(range_magnitude[:, 0])
                ax1.relim()
                ax1.autoscale_view()

                plot_adc_data(ax2, line_real, line_imag, adc_data, num_samples, num_chirps, frame_index, chirp_index, rx_channel, frame_periodicity, sample_rate, idle_time, adc_start_time, ramp_time, freq)

                plot_2d_fft_3d(ax3, adc_data, num_samples, num_chirps, rx_channel, frame_index, range_res)
                plot_2d_fft_image(ax4, adc_data, num_samples, num_chirps, num_frames, rx_channel, frame_index, range_res)

                range_doppler_data = compute_range_doppler(adc_data, num_samples, num_chirps, num_frames)
                plot_range_doppler(ax6, range_doppler_data, v_max, v_res, num_samples, num_chirps, range_res, frame_index)

                plt.draw()
                plt.pause(0.1)  # We dont need to have this if we are reading real time
            ...
            if running_chirp:
                plot_adc_data(ax_chirp, line_real_chirp, line_imag_chirp, adc_data, num_samples, num_chirps, frame_index2, chirp_index, rx_channel, frame_periodicity, sample_rate, idle_time, adc_start_time, ramp_time, freq)
                plt.draw()
                plt.pause(0.1)
                
                chirp_index = (chirp_index + 1) % num_chirps 

                if chirp_index == 0:
                    frame_index2 = (frame_index2 + 1) % num_frames
            else:
                plt.pause(0.1)  

            if len(timestamps) > 1:
                unique_periods, counts = analyze_periods(timestamps)
                print(unique_periods, counts)
                plot_periods(ax_periods, unique_periods, counts)
                next_interference_time, lower_bound, upper_bound = predict_next_interference(timestamps, unique_periods, counts, frame_periodicity, idle_time, adc_start_time)
                plot_prediction(ax_prediction, next_interference_time, lower_bound, upper_bound, timestamps, frame_periodicity, idle_time, adc_start_time, unique_periods, counts)
                plt.draw()

        print("Timestamps when amplitude went beyond threshold:", timestamps)
